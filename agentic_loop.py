...
def observe_live_endpoints():
    ...
    try:
        response = requests.get(
            "http://127.0.0.1:5000/students/by-id?student_id=abc",
            timeout=5
        )
        results.append(
            f"/students/by-id?student_id=abc -> HTTP {response.status_code}, Response: {response.text}"
        )
    except Exception as exc:
        results.append(
            f"/students/by-id?student_id=abc -> error: {exc}"
        )
